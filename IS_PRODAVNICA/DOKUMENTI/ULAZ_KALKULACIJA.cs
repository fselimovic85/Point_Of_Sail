...
namespace IS_PRODAVNICA.DOKUMENTI
{
    public partial class ULAZ_KALKULACIJA : Form
    {
        ...
        private void btnPretraga_Zaglavlje_Kalkulacije_Click(object sender, EventArgs e)
        {
        ...
        //Ucitavanje Poslovnih Partnera u ComboBox
        ...
        public void Unesi_Zaglavlje()
        {
            try
            {
                ...
                cm = new SqlCommand(" SELECT * " +
                                    ...
                                    " WHERE  broj_dokumenta='" + textBrojDokumenta.Text + "' AND vrsta_dokumenta=" +
                                    "'Ulaz-Kalkulacija'", cn);
                cm.ExecuteNonQuery();
                dr = null;
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
                    ...
                    {
                        MessageBox.Show(" Postoji već Kalkulacija sa datim brojem dokumenta ", " Upozorenje !!!",
        ...
        public void Ucitaj_Zaglavlje()
        {
            ...
            cm = new SqlCommand(" Select zd.id as id, zd.broj_dokumenta as broj_dokumenta, zd.nacin_placanja as nacin_placanja, " +
                                ...
                                " Where zd.vrsta_dokumenta='Ulaz-Kalkulacija' " +
                                " order by zd.id", cn);
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        //Ucitavanje detalja dokumenta
        public void Ucitaj_Detalje_Dokumenta()
        {
            int i = 0;
            dataGrid_Detalji_Kalkilacje.Rows.Clear();
            cn.Open();
            ...
            {
                i += 1;
                dataGrid_Detalji_Kalkilacje.Rows.Add(i, dr["id"].ToString(), dr["sifra_artikla"].ToString(),
            ...
            if (dataGrid_Detalji_Kalkilacje.Rows.Count != 0 && dataGrid_Detalji_Kalkilacje.Rows.Count != 1)
            {
                //Selekcija posledljeg reda i celije po redu, u za u gridu
                int Indeks_Reda = dataGrid_Detalji_Kalkilacje.Rows.Count - 1;
                int Indeks_Kolone = dataGrid_Detalji_Kalkilacje.Columns.Count - 1;

                dataGrid_Detalji_Kalkilacje.Rows[Indeks_Reda].Selected = true;
                dataGrid_Detalji_Kalkilacje.Rows[Indeks_Reda].Cells[Indeks_Kolone].Selected = true;

                //In case if you want to scroll down as well.
                dataGrid_Detalji_Kalkilacje.FirstDisplayedScrollingRowIndex = Indeks_Reda;
            }
        }
        //Unos podataka u detalje dokumenta
        ...
        private void btnUcitaj_Zaglavnje_Kalkulacije_Click(object sender, EventArgs e)
        {
            Ucitaj_Zaglavlje();
        }

        private void dataGrid_Zaglavlje_Kalkulacije_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            id_zaglavlja_dokumenta = Int32.Parse(dataGrid_Zaglavlje_Kalkulacije.Rows[e.RowIndex].Cells[1].Value.ToString());
            Ucitaj_Detalje_Dokumenta();
        }
