...
namespace IS_PRODAVNICA.MATICNI_PODACI
{
    public partial class ARTIKLI : Form
    {
        ...
        public ARTIKLI()
        {
            ...
            cmbJednica_Mere.Items.Add("--Selektuj Jedinicu Mere--");
            cmbJednica_Mere.Items.Add("KG");
            cmbJednica_Mere.Items.Add("M");
            cmbJednica_Mere.Items.Add("L");
            cmbJednica_Mere.Items.Add("Kom.");
            //Ispisivanje prvog reda
            cmbJednica_Mere.SelectedIndex = 0;
        ...
        public void Dodaj_Artikal()
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    {
                        MessageBox.Show(" Postoji već Artikal sa datom šifrom, proverite upis ", " Upozorenje !!!",
                ...
                if (textSifra_Artikla.Text != "")
                {
                    ...
                    cn.Open();
                    cm = new SqlCommand(" INSERT INTO ARTIKAL ( sifra_artikla, naziv_artikla, jedinica_mere, stopa_poreska, vrsta_artikla, barkod ) " +
                                        " VALUES ( @sifra_artikla, @naziv_artikla, @jedinica_mere, @stopa_poreska, @vrsta_artikla, @barkod) ", cn);
                    cm.Parameters.AddWithValue("@sifra_artikla", textSifra_Artikla.Text.ToString());
                    cm.Parameters.AddWithValue("@naziv_artikla", texNaziv_Artikla.Text);
                    cm.Parameters.AddWithValue("@jedinica_mere", cmbJednica_Mere.SelectedItem);
                    ...
                    texNaziv_Artikla.Clear();
                    textSifra_Artikla.Clear();
                    cmbJednica_Mere.SelectedIndex = 0;
                    cmbStopa_Poreska.SelectedIndex = 0;
                    cmbVrsta_Artikla.SelectedIndex = 0;
                    textBarkod.Clear();
                    texNaziv_Artikla.Focus();
                    cn.Close();
                }
                else
                {
        ...
        private void btnUcitaj_Artikle_Click(object sender, EventArgs e)
        {
            Ucitaj_Artikle();
            texNaziv_Artikla.Focus();
        }

        private void btnDodaj_Artikal_Click(object sender, EventArgs e)
        {
            Dodaj_Artikal();
            texNaziv_Artikla.Focus();
        }
        ...
        {
            string ime_kolone_artkla = dataGrid_Artikli.Columns[e.ColumnIndex].Name;
            artikal_id= dataGrid_Artikli.Rows[e.RowIndex].Cells[1].Value.ToString();

            //dodeljivanje vrednosti id artikla u int obliku
            id_artikla = Int32.Parse(artikal_id);

            if (ime_kolone_artkla == "Edit_Artikla")
            {
                textSifra_Artikla.Text = dataGrid_Artikli.Rows[e.RowIndex].Cells[2].Value.ToString();
                texNaziv_Artikla.Text = dataGrid_Artikli.Rows[e.RowIndex].Cells[3].Value.ToString();
                cmbJednica_Mere.SelectedItem = dataGrid_Artikli.Rows[e.RowIndex].Cells[4].Value.ToString();
            ...
            else if (ime_kolone_artkla == "Delete_Artikla")
            {
                try
                {
                    if (MessageBox.Show("Da li ste sigurni da želite da izbrišete selektovani red?", "Brisanje Artikla", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        cn.Open();
        ...
        public void Izmeni_Artikal()
        {
            if (MessageBox.Show("Želite li zaista za izmenite podatke o Artiklu", "Podaci o Artiklu", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                while (dr.Read())
                {
                    if (textSifra_Artikla.Text == dr["sifra_artikla"].ToString())
                    {
                        MessageBox.Show("Postoji radnik sa datom sifrom, molimo Vas unesite drugu sifru",
                ...
                if (textSifra_Artikla.Text != "")
                {
                    ...
                    cm = new SqlCommand(" UPDATE ARTIKAL set  sifra_artikla=@sifra_artikla, " +
                    ...
                    cm.Parameters.AddWithValue("@vrsta_artikla", cmbVrsta_Artikla.SelectedItem);
                    cm.Parameters.AddWithValue("@barkod", textBarkod.Text);
                    cm.Parameters.AddWithValue("@id", id_artikla);
                    cm.ExecuteNonQuery();
                    cn.Close();
        ...
        private void btnIzmeni_Artikl_Click(object sender, EventArgs e)
        {
            Izmeni_Artikal();
            Ucitaj_Artikle();
        }
        ...
        //Pretraga Artikla
        private void Pretraga_Artikla()
        {
            int i = 0;
            dataGrid_Artikli.Rows.Clear();
            cn.Open();
            cm = new SqlCommand("Select * from ARTIKAL " +
                                " WHERE naziv_artikla LIKE '%" + textPretraga_Artikla.Text + "%'", cn);
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
                i += 1;
        ...
        private void btnPretraga_Artikla_Click(object sender, EventArgs e)
        {
            Pretraga_Artikla();
        }
    }
