...
namespace IS_PRODAVNICA.DOKUMENTI
{
    ...
    {
        //Objekti koji su potrebni radi uspostvaljanja konekcije
        ...
        public RADNICI()
        {
            ...
            //Dodavanje vrsta radnika u ComboBox()
        ...
        public void Dodaj_radnika()
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    {
                        MessageBox.Show(" Postoji već radnik sa datim JMBG-om, proverite upis ", " Upozorenje !!!",
        ...
        public void Pretrazi_Radnike()
        {
            ...
            while (dr.Read())
            {
                ...
                dataGrid_Radnici.Rows.Add(i, dr["id"].ToString(), dr["ime_prezime"].ToString(),
                                             dr["JMBG"].ToString(), dr["vrsta_radnika"].ToString(),
                                             dr["korisnicko_ime"].ToString(), dr["lozinka"].ToString(),
                                             dr["adresa"].ToString(), dr["mesecna_zarada"].ToString()
                                             );
            }
            dr.Close();
            cn.Close();
        ...
        public void Izmeni_Radnika()
        {
            if (MessageBox.Show("Želite li zaista za izmenite podatke o radniku", "Imena podataka o radniku", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                while (dr.Read())
                {
                    if (textJMBG.Text == dr["JMBG"].ToString())
                    {
                        MessageBox.Show("Postoji radnik sa datim JMBG, unesite novi JMBG",
        ...
        private void btnNovi_Grad_Click(object sender, EventArgs e)
        {
            Dodaj_radnika();
        }
                ...
                try
                {
                    if (MessageBox.Show("Da li ste sigurni da želite da izbrišete selektovani red?","Brisanje Radnika", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        cm = new SqlCommand(" DELETE FROM RADNIK " +
                                            " WHERE id=" + dataGrid_Radnici.Rows[e.RowIndex].Cells[1].Value.ToString() + "", cn);
                        cm.ExecuteNonQuery();
                        cn.Close();
        ...
        private void btnPretraga_Radnika_Click(object sender, EventArgs e)
        {
            Pretrazi_Radnike();
        }
    }
